...
<div style="margin-left: 8%; margin-right: 8%">
  ...
  <span class="light">
    Closures defined by letting m = M(x) for some variable "m" and any value "x" (see below)  are recursive in the sense that: <br>
    ...
    &nbsp;&nbsp;&nbsp; (3) the current value of x is preserved in permanent storage.
  </span>
  <p> Regarding the last item and x retrieved from storage: If m is defined, then <span> `m(()=>x)(newFunc1)(newFunc2)</span> ... Otherwise, redefine m with "m = M(x) using the most recent value of x.</p>
  ...
  <span class="teaser"
    ...
    Case 1 -- Clicking on the three visible sides of the cube.</span>
     <p>The visible cube interface gives access to all six sides. Clicking the center of each face causes rotation of the whole cube around the axis perpendicular to that center. Everything moves except for clicked center and its counterpart on the opposite side</p> 

  <p> x in the Rubik's cube application is an array of six nine-member arrays of references to the strings "blue, green, red, orange, yellow, and white." These strings correspond to the names of CSS selectors, and the "background-color" properties of each. Here's what I mean:
    
<pre>{css}</pre> 

<p> The 54 divs that combine to represent the cube in the DOM are shown below. The visual representation that appears in the browser is on the next page, <a href = "./cube7">Rubik's cube</a> </p> 

<p> Notice that the front, top, and right side representations contain buttons. Go to <a href = "./cube7">Rubik's cube</a>, click the "Start" button or press the 'V' key to restore the cube to it to the "solved" configuration, then click the upper left corner of the right side of the cube. The right face rotates counterclockwise 90 degrees pursuant to the "on:click" instruction seen in the DOM representation (below). "m = m(Rz)" executes. "m(Rz)" updates x in the m-M(x) closure; the "m =" part triggers reactivity, causing the update to immediately appear in the browser." </p>  
<p> Now click on the top center three times. The right side should be green now. Rotate the right side counterclockwise by clicking on its upper left corner. Now click the top center once to restore the cube to its original orientation.</p>
<span style> Digression: <p>This section is about clicking on cube image, but when we get to it later, you will see that pressing the "R" key or clicking the button labeled "R" rotates whatever face happens to be on the right when the "R" key is pressed.or the "R" button is clicked. </p>
<p> </p>
<pre>{dom}</pre>
  ...
  <span class="teaser"
    ...
    <p> Similar to clicking on parts of the cube, the callback specified in buttons that rotate faces, middle sections, and the entire cube do so by calling m(func) for some function "func" that rearranges arrays of six nine-element arrays. This results in x mutating into fu on  the same functions that are invoked when the the click is on a section of the cube. For example, </p>
<pre>{example3}</pre>
  <span class="teaser"
    >***********************************************************************
    <br> 
    Case 3 -- Key presses. </span>
<p> Pressing keys invokes eventHandler(e) where "e" is the key-press event and eventHandler is defined as follows:</p>  
<pre>{handleEventCode}</pre>
<p> "svelte:window on:keypress =  &#123;  handleEvent &#125; " routes keyPress event objects to handleEvent.</p>
  <p> The <a href="./cube7">Rubik's Cube simulation</a> features a m-M(x) closure
  ...
  <div class="background">
    ...
    <p style="color: lightgreen;">
      Indiana University awarded me a bachelor of science degree in chemistry, a
      ...
      middle class and people with benefactors to pay the premiums.
    </p>
    <p style="color: lightgreen;">
    ...
    <p>
      ...
      numerical computations, M holds the ever-changing state of play in the<a
        href="./score">Game of Score</a
      >, but that application relies too heavily on the functions that transform
      x in the m-M(x) closure. A modified version of M will make the code more
      efficient and understandable after I refactor the application.
    </p>
    ...
    <p>
      ...
      to transform them by means of the functions m fetches. The <a
        href="./cube7">Simulated Rubik's cube</a
      > is a case in point.
    </p>

    <p>
      The array x in the m-M(x) Rubik's cube array is an array consisting of six
    ...
    <p id = "cow7">
      To understand the interface between the m-M(x) representation of the cube
      with the 54 colored divs in the DOM, we first need to know how x in the
      closure is defined. Here it is:
    </p>
    ...
    <p>
      ...
      ranging from m(dF3x)[0][0] to m(dF3)[5][8].
    </p>

    <p>
      R specifies that whatever string reference happens to be in position
      ...
      Here's the definition of R:
    </p>

    <pre>{Rcode}</pre>
    <p> When R returns temp, the transformation of x completes. It's new value is temp since x = Func(x) and Func is R in this instance.  
    </p>
...
paypal card denominations
    ...
    <p>
      ...
      my current project, <a href="./cube7">Simulated Rubik's cube</a>.
    </p>

    <p>
      Key press and mouse click events call eventHandler(e) where e is an object
      ...
      function that rearranges resides response to events initiated in the DOM.
    </p>
    <p>
